...
def get_complex_data(verts,faces,k_eig=128):
    # compute connection laplacian and eigen-decomposition
    ...
    if is_cloud:
        # L, M = robust_laplacian.point_cloud_laplacian(verts_np)
        # massvec_np = M.diagonal()
        raise RuntimeError('Not implentation on Point Cloud')
    else:
        # L, M = robust_laplacian.mesh_laplacian(verts_np, faces_np)
        # massvec_np = M.diagonal()
        L = pp3d.MeshVectorHeatSolver(verts_np,faces_np).get_connection_laplacian()
        massvec_np = pp3d.vertex_areas(verts_np, faces_np)
        massvec_np += eps * np.mean(massvec_np)
    ...
    # === Convert back to torch
    # massvec = torch.from_numpy(massvec_np).to(device=device, dtype=dtype)
    evals = torch.from_numpy(evals_np).to(device=device)
    evecs = torch.from_numpy(evecs_np).to(device=device)
