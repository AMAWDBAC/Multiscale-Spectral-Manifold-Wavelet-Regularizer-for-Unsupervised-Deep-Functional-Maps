...
class MatchingDataset(Dataset):
    def __init__(self,root_dir,train,k_eig=128,use_cache=True,\
        ...
        # compute wks descriptors and complex spectral data
        for s in range(len(self.evals_list)):
            self.descs_list.append(diffusion_net.geometry.auto_WKS(self.evals_list[s], self.evecs_list[s], 128).float())
    ...
    def __getitem__(self, index):
        ...
        verts_x,descs_x,massvec_x,evals_x,evecs_x,gradX_x,gradY_x,L_x=self.get_data(idx1)
        verts_y,descs_y,massvec_y,evals_y,evecs_y,gradX_y,gradY_y,L_y=self.get_data(idx2)
        

        return verts_x,descs_x,massvec_x,evals_x,evecs_x,gradX_x,gradY_x,L_x,\
            verts_y,descs_y,massvec_y,evals_y,evecs_y,gradX_y,gradY_y,L_y

    def get_data(self, idx):
        return self.verts_list[idx],self.descs_list[idx],self.massvec_list[idx],self.evals_list[idx],\
            self.evecs_list[idx],self.gradX_list[idx],self.gradY_list[idx],self.L_list[idx],\
